...
// Render todos nicely
function TodosDisplay({ todos }: { todos: Todo[] }): JSX.Element {
...
function FileContentPreview({ content }: { content: string; path?: string }): JSX.Element {
...
function FileEditSummary({ args }: { args: Record<string, unknown> }): JSX.Element | null {
...
// Command display
function CommandDisplay({ command, output }: { command: string; output?: string }): JSX.Element {
  return (
...
export function ToolCallRenderer({
  ...
  const renderFormattedContent = (): JSX.Element | null => {
    if (!args) return null
  ...
  const renderFormattedResult = (): JSX.Element | null => {
